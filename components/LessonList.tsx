...
const ChevronIcon = () => (
  <svg {...iconProps} className="shrink-0 text-slate-400 transition-transform group-open:rotate-90">
  ...
  return (
    <div>
      {groups.map(([category, inCategory]) => {
        const doneCount = inCategory.filter((l) => finished.has(l.setId)).length;
